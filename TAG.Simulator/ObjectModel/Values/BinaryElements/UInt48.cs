...
namespace TAG.Simulator.ObjectModel.Values.BinaryElements
{
	...
	/// UInt48 value.
	/// </summary>
	public class UInt48 : SimulationNode, IBinaryElement
	{
		...
		public UInt48(ISimulationNode Parent, Model Model)
			: base(Parent, Model)
		{
		}
		...
		/// </summary>
		public override string LocalName => "UInt48";
		...
		{
			return new UInt48(Parent, Model);
		}
		...
		public async Task Append(MemoryStream Output, Variables Variables)
		{
			...
			Output.Write(BitConverter.GetBytes(this.value ?? 
				((Result = await this.script.EvaluateAsync(Variables)) is System.UInt64 Value ? Value :
				Convert.ToUInt64(Result))), 0, 6);
		}
